...
class GitCommitHelper:
    ...
    def check_coverage(self):
        ...
        process = subprocess.run(['pytest', '--cov=app', '--cov=tools', '--cov-report=term-missing'], capture_output=True)
        coverage_output = process.stdout.decode()
        self.coverage_passed = "TOTAL" in coverage_output and "100%" in coverage_output
