...
def test_check_coverage(mock_subprocess_run, mocker):
    ...
    assert helper.check_coverage() is True
...
def test_generate_commit_message(mocker):
    ...
    message = helper.generate_commit_message(diff)
...
def test_create_commit(mocker):
    ...
    helper.create_commit("Commit message")
...
def test_main_no_changes(mocker):
    helper = GitCommitHelper()
    mocker.patch.object(helper, 'get_git_diff', return_value="")
    helper.main()
...
def test_main(mocker, input_side_effect, prompt_return_value):
    ...
    mocker.patch.object(helper, 'run_tests', return_value=("test output", 0))
    mocker.patch.object(helper, 'check_coverage', return_value=True)
    mocker.patch.object(helper, 'generate_commit_message', return_value="Generated commit message")
    mocker.patch.object(helper, 'create_commit')
